...
${sectionHtml}
<div class="footer">This report is for personal investment research only and does not constitute financial advice.</div>
</body>
</html>`;
}
...
export async function generateAndSharePdf(opts: ReportOptions): Promise<string> {
  const html = buildHtml(opts);
  const { uri } = await Print.printToFileAsync({ html, base64: false });

  if (await Sharing.isAvailableAsync()) {
    await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: opts.title });
  }

  return uri;
}

export function buildThesisReport(thesis: {
  ticker?: string;
  title: string;
  direction: string;
  conviction: number;
  ...
  const sections: ReportSection[] = [
    ...
    {
      title: 'Risk Factors',
      content: `<ul>${thesis.risks.map((r) => `<li class="risk-item">${escapeHtml(r)}</li>`).join('')}</ul>`,
      isHtml: true,
    },
  ];
